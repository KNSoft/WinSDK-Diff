    ...
    ) throw()
{
    ...
    {
        drive,     drive     ? static_cast<size_t>(_MAX_DRIVE) : 0,
        directory, directory ? static_cast<size_t>(_MAX_DIR) : 0,
        file_name, file_name ? static_cast<size_t>(_MAX_FNAME) : 0,
        extension, extension ? static_cast<size_t>(_MAX_EXT) : 0
    };
