...
#define IDDCX_VERSION_SELENIUM 0x1B00
#define IDDCX_VERSION_SELENIUM_MODECACHE_FIX 0x1B01
#define IDDCX_VERSION_LATEST IDDCX_VERSION_SELENIUM
...
enum IDDCX_ADAPTER_FLAGS : UINT
{
    ...
    /// Note: This flag is deprecated for IddCx 1.11 and later drivers.
    /// </summary>
...
enum IDDCX_MONITOR_DESCRIPTION_TYPE : UINT
{
    ...
    /// <summary>
    /// The monitor description is DisplayID only. This means a pure DisplayID block, rather than one embedded inside an
    /// EDID block, and no EDID blocks preceding or following.
    /// </summary>
...
enum IDDCX_METADATA2_VALID_FLAGS : UINT
{
    ...
    /// <summary>
    /// When a driver has associated an ID3D12Device object with a swapchain in a call to IddCxSwapChainSetDevice2
    /// this flag will be set to indicate the IDDCX_METADATA2::pD3D12Surface field contains valid data.
    /// Note that IDDCX_METADATA2::pSurface will be null in this situation.
    /// </summary>
...
enum IDDCX_DEVICE_FEATURES_1_11
{
    IDDCX_DEVICE_FEATURES_1_11_NONE = 0,
    /// <summary>
    /// The underlying OS supports D3D12 swapchains.
    /// </summary>
    IDDCX_DEVICE_FEATURES_1_11_D3D12 = 0x1,
    /// <summary>
    /// The underlying OS supports the driver submitting DisplayID only monitor descriptors.
    /// </summary>
    IDDCX_DEVICE_FEATURES_1_11_DISPLAY_ID_ONLY = 0x2,
    /// <summary>
    /// The underlying OS supports updating the StaticDesktopReencodeFrameCount value for an adapter.
    /// </summary>
    IDDCX_DEVICE_FEATURES_1_11_UPDATE_REENCODE_COUNT = 0x4,
};
DEFINE_ENUM_FLAG_OPERATORS(IDDCX_DEVICE_FEATURES_1_11);

enum IDDCX_SWAPCHAIN_DEVICE_TYPE
{
    IDDCX_SWAPCHAIN_DEVICE_TYPE_INVALID = 0,
    /// <summary>
    /// This value is used when the driver wants to set IDARG_IN_SWAPCHAINSETDEVICE2::Driver.pDxgiDevice
    /// </summary>
    IDDCX_SWAPCHAIN_DEVICE_TYPE_DXGI = 1,
    /// <summary>
    /// This value is used when the driver wants to set IDARG_IN_SWAPCHAINSETDEVICE2::Driver.pD3d12Device
    /// </summary>
...
struct IDARG_OUT_FEATURES_SUPPORTED;
...
/// <summary>
/// Driver DDI function that is called by OS to send data to an I2C device in a monitor
/// </summary>
...
/// <summary>
/// Driver DDI function that is called by OS to send and return data received from an I2C device in a monitor
/// </summary>
/// <param name="MonitorObject">A handle to monitor object to send I2C data.</param>
/// <param name="pInArgs">Input arguments of function</param>
/// <returns>If the routine succeeds it return STATUS_SUCCESS otherwise an appropriate error code</returns>
/// <remarks>
/// The data is transmitted to and received from an I2C device in the monitor that is connected to path specified by
/// PathIdx.
///
/// EVT_IDD_CX_MONITOR_I2C_TRANSMIT_AND_RECEIVE is responsible for signaling the I2C start condition, sending the I2C
/// transmit address, sending the data in the buffer, checking for acknowledgments from the receiver, sending the I2C
/// receive address, receiving the data from the I2C device, sending acknowledgments and signaling the stop
/// condition.
///
/// For details about the I2C bus, see the I2C Bus Specification, published by Philips Semiconductors. The specification
/// defines a protocol for initiating I2C communication, reading and writing bytes over the I2C data line, and terminating
/// I2C communication. (This resource may not be available in some languages and regions.)
///
/// EVT_IDD_CX_MONITOR_I2C_TRANSMIT_AND_RECEIVE is required to transmit data to an I2C device that has address 0x6E but
/// is permitted to refuse to transmit data to any I2C device that has a different address.
///
/// EVT_IDD_CX_MONITOR_I2C_TRANSMIT_AND_RECEIVE is required to receive data from an I2C device that has address 0x6F but is
/// permitted to refuse to receive data from any I2C device that has a different address.
///
/// EVT_IDD_CX_MONITOR_I2C_TRANSMIT_AND_RECEIVE is permitted to block if another part of the display driver or graphics
/// hardware is using the specified monitor's I2C bus. It is also permitted to block if the display driver is using the
/// I2C bus to send or receive High-bandwidth Digital Content Protection (HDCP) data.
///
/// If the display adapter supports HDCP, the EVT_IDD_CX_MONITOR_I2C_TRANSMIT_AND_RECEIVE function must refuse to send
/// data to an I2C device if the device has an I2C address that is used by HDCP.
///
/// EVT_IDD_CX_MONITOR_I2C_TRANSMIT_AND_RECEIVE must never transmit data to or receive data from an I2C device on the
/// display adapter. That is, this function can transmit data to an I2C device in a monitor that is connected to the 
/// display adapter, but not to an I2C device that is on the display adapter itself.
/// </remarks>
typedef
_Function_class_(EVT_IDD_CX_MONITOR_I2C_TRANSMIT_AND_RECEIVE)
_IRQL_requires_same_
NTSTATUS
NTAPI
EVT_IDD_CX_MONITOR_I2C_TRANSMIT_AND_RECEIVE(
    ...
    IDDCX_MONITOR MonitorObject,
    _In_
    const IDARG_IN_I2C_TRANSMIT_AND_RECEIVE* pInArgs
    );

typedef EVT_IDD_CX_MONITOR_I2C_TRANSMIT_AND_RECEIVE *PFN_IDD_CX_MONITOR_I2C_TRANSMIT_AND_RECEIVE;
...
struct IDDCX_MONITOR_DESCRIPTION
{
    ...
    /// Depending on the Type it can be either EDID, DisplayID + EDID, or DisplayID
...
struct IDARG_IN_RELEASEANDACQUIREBUFFER2
{
    ...
    /// <summary>
    /// A pointer to the ID3D12CommandQueue object the driver will use to access the surface aquired.
    /// </summary>
...
struct IDDCX_METADATA2
{
    ...
    /// <summary>
    /// A D3D12 surface that contains the image to encode and transmit. The driver can use this surface anytime until IddCxSwapChainReleaseAndAcquire is called again.
    /// </summary>
...
struct IDARG_OUT_FEATURES_SUPPORTED
{
    ...
    /// <summary>
    /// The IddCx 1.11 features supported by the OS.
    /// </summary>
    IDDCX_DEVICE_FEATURES_1_11 Features_1_11;
};
...
struct IDARG_IN_I2C_TRANSMIT_AND_RECEIVE
{
    ...
    /// <summary>
    /// [in] The size, in bytes, of the buffer pointed to by pTransmitData, this parameter must be between 1 and 64, inclusive
    /// </summary>
    ...
    /// [in] A pointer to a buffer that holds the data to be transmitted
    /// </summary>
    _Field_size_full_(TransmitDataSizeInBytes) PVOID pTransmitData;
    ...
    /// [out] A pointer to a buffer that receives the data.
    /// </summary>
    _Field_size_full_(ReceiveDataSizeInBytes) PVOID pReceiveData;
};
#pragma endregion
...
/// </summary>
/// <param name="SwapChainObject">The swap-chain object passed to the EVT_IDD_CX_MONITOR_ASSIGN_SWAPCHAIN call.</param>
/// <param name="pOutArgs">Output arguments of function</param>
/// <returns>
...
/// Note: This DDI is deprecated for IddCx 1.11 and later drivers.
/// </summary>
/// <param name="SwapChainObject">The swap-chain object passed to the EVT_IDD_CX_MONITOR_ASSIGN_SWAPCHAIN call.</param>
/// <param name="pSurfaceAddresss">Output arguments of function</param>
/// <returns>
/// For IddCx 1.11 and later drivers this DDI will return E_NOTIMPL or another error code.
...
//
// IDD Function: IddCxCheckOsFeatureSupport
//
typedef
_Must_inspect_result_
WDFAPI
NTSTATUS
(NTAPI *PFN_IDDCXCHECKOSFEATURESUPPORT)(
    _In_
    PIDD_DRIVER_GLOBALS DriverGlobals,
    _Out_
    IDARG_OUT_FEATURES_SUPPORTED* pOutArgs
    );

/// <summary>
/// A function the driver must call to determine OS support for features
/// </summary>
/// <param name="pOutArgs">Output arguments to the function</param>
/// <returns>If the routine succeeds it return STATUS_SUCCESS otherwise an appropriate error code</returns>
_Must_inspect_result_
...
{
#if defined(IDD_EVERYTHING_ALWAYS_AVAILABLE)
    return ((PFN_IDDCXCHECKOSFEATURESUPPORT) IddFunctions[IddCxCheckOsFeatureSupportTableIndex])(IddDriverGlobals, pOutArgs);
#else
    if (IDD_IS_FUNCTION_AVAILABLE(IddCxCheckOsFeatureSupport)) {
        return ((PFN_IDDCXCHECKOSFEATURESUPPORT) IddFunctions[IddCxCheckOsFeatureSupportTableIndex])(IddDriverGlobals, pOutArgs);
    }
    else {
        return ((PFN_WDFDRIVERERRORREPORTAPIMISSING) WdfFunctions[WdfDriverErrorReportApiMissingTableIndex])(WdfDriverGlobals, WdfGetDriver(), IddFrameworkExtensionName, IddCxCheckOsFeatureSupportTableIndex, TRUE);
    }
#endif
}

//
// IDD Function: IddCxSwapChainSetDevice2
//
typedef
_Must_inspect_result_
WDFAPI
HRESULT
(NTAPI *PFN_IDDCXSWAPCHAINSETDEVICE2)(
    _In_
    PIDD_DRIVER_GLOBALS DriverGlobals,
    _In_
    IDDCX_SWAPCHAIN SwapChainObject,
    _In_
    CONST IDARG_IN_SWAPCHAINSETDEVICE2* pInArgs
    );

/// <summary>
/// An OS callback function the driver calls within its SetSwapChain routine to setup the swap-chain with a specific device
/// based on the rendering API they wish to use to access surfaces.
/// </summary>
/// <param name="SwapChainObject">The swap-chain object passed to the EVT_IDD_CX_MONITOR_ASSIGN_SWAPCHAIN call.</param>
/// <param name="pInArgs">Input arguments to the function</param>
/// <returns>If the routine succeeds it return S_OK otherwise an appropriate error code.</returns>
...
{
#if defined(IDD_EVERYTHING_ALWAYS_AVAILABLE)
    return ((PFN_IDDCXSWAPCHAINSETDEVICE2) IddFunctions[IddCxSwapChainSetDevice2TableIndex])(IddDriverGlobals, SwapChainObject, pInArgs);
#else
    if (IDD_IS_FUNCTION_AVAILABLE(IddCxSwapChainSetDevice2)) {
        return ((PFN_IDDCXSWAPCHAINSETDEVICE2) IddFunctions[IddCxSwapChainSetDevice2TableIndex])(IddDriverGlobals, SwapChainObject, pInArgs);
    }
    else {
        ((PFN_WDFDRIVERERRORREPORTAPIMISSING) WdfFunctions[WdfDriverErrorReportApiMissingTableIndex])(WdfDriverGlobals, WdfGetDriver(), IddFrameworkExtensionName, IddCxSwapChainSetDevice2TableIndex, FALSE);
        return (HRESULT)0;
    }
#endif
}
...
//
// IDD Function: IddCxSwapChainUpdateStaticDesktopReencodeFrameCount
//
typedef
_Must_inspect_result_
WDFAPI
HRESULT
(NTAPI *PFN_IDDCXSWAPCHAINUPDATESTATICDESKTOPREENCODEFRAMECOUNT)(
    _In_
    PIDD_DRIVER_GLOBALS DriverGlobals,
    _In_
    IDDCX_SWAPCHAIN SwapChainObject,
...
/// <summary>
/// An OS callback function the driver calls whwen it wants to set the reencode count for the specified swap chain to
/// somthing different than the value specified in IDDCX_ADAPTER_CAPS::StaticDesktopReencodeFrameCount.
/// </summary>
/// <param name="SwapChainObject">The swap-chain object passed to the EVT_IDD_CX_MONITOR_ASSIGN_SWAPCHAIN call.</param>
/// <param name="StaticDesktopReencodeFrameCount">The new reencode count to use for the specified swapchain.</param>
/// <returns>If the routine succeeds it return S_OK otherwise an appropriate error code.</returns>
_Must_inspect_result_
FORCEINLINE
HRESULT
...
{
#if defined(IDD_EVERYTHING_ALWAYS_AVAILABLE)
    return ((PFN_IDDCXSWAPCHAINUPDATESTATICDESKTOPREENCODEFRAMECOUNT) IddFunctions[IddCxSwapChainUpdateStaticDesktopReencodeFrameCountTableIndex])(IddDriverGlobals, SwapChainObject, StaticDesktopReencodeFrameCount);
#else
    if (IDD_IS_FUNCTION_AVAILABLE(IddCxSwapChainUpdateStaticDesktopReencodeFrameCount)) {
        return ((PFN_IDDCXSWAPCHAINUPDATESTATICDESKTOPREENCODEFRAMECOUNT) IddFunctions[IddCxSwapChainUpdateStaticDesktopReencodeFrameCountTableIndex])(IddDriverGlobals, SwapChainObject, StaticDesktopReencodeFrameCount);
    }
    else {
        ((PFN_WDFDRIVERERRORREPORTAPIMISSING) WdfFunctions[WdfDriverErrorReportApiMissingTableIndex])(WdfDriverGlobals, WdfGetDriver(), IddFrameworkExtensionName, IddCxSwapChainUpdateStaticDesktopReencodeFrameCountTableIndex, FALSE);
        return (HRESULT)0;
    }
#endif
}
