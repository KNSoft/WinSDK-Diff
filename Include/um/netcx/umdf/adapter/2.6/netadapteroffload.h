...
typedef struct _NET_ADAPTER_OFFLOAD_RSC_CAPABILITIES
{
    ...
    // NetAdapterOffloadLayer4FlagTcpNoOptions and NetAdapterOffloadLayer4FlagUdp
    // are the only valid flag values to indicate TCP and UDP support.
    //
    NET_ADAPTER_OFFLOAD_LAYER4_FLAGS
        Layer4Flags;
...
//
// NET Function: NetOffloadIsUdpRscEnabled
//
typedef
_IRQL_requires_max_(PASSIVE_LEVEL)
WDFAPI
    ...
    )
{
    ...
    return ((PFN_NETOFFLOADISUDPRSCENABLED) NetFunctions[NetOffloadIsUdpRscEnabledTableIndex])(NetDriverGlobals, Offload);
#else
    if (NET_IS_FUNCTION_AVAILABLE(NetOffloadIsUdpRscEnabled)) {
        return ((PFN_NETOFFLOADISUDPRSCENABLED) NetFunctions[NetOffloadIsUdpRscEnabledTableIndex])(NetDriverGlobals, Offload);
    }
    else {
        ((PFN_WDFDRIVERERRORREPORTAPIMISSING) WdfFunctions[WdfDriverErrorReportApiMissingTableIndex])(WdfDriverGlobals, WdfGetDriver(), NetFrameworkExtensionName, NetOffloadIsUdpRscEnabledTableIndex, FALSE);
        return (BOOLEAN)0;
    }
#endif
}
