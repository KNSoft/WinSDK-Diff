...
#else
    if (NET_IS_FUNCTION_AVAILABLE(NetAdapterSetReceiveScalingCapabilities)) {
        ((PFN_NETADAPTERSETRECEIVESCALINGCAPABILITIES) NetFunctions[NetAdapterSetReceiveScalingCapabilitiesTableIndex])(NetDriverGlobals, Adapter, Capabilities);
    }
    else {
        ((PFN_WDFDRIVERERRORREPORTAPIMISSING) WdfFunctions[WdfDriverErrorReportApiMissingTableIndex])(WdfDriverGlobals, WdfGetDriver(), NetFrameworkExtensionName, NetAdapterSetReceiveScalingCapabilitiesTableIndex, FALSE);
    }
#endif
}
