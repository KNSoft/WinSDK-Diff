...
#else
    if (NET_IS_FUNCTION_AVAILABLE(NetDeviceInitSetPowerPolicyEventCallbacks)) {
        ((PFN_NETDEVICEINITSETPOWERPOLICYEVENTCALLBACKS) NetFunctions[NetDeviceInitSetPowerPolicyEventCallbacksTableIndex])(NetDriverGlobals, DeviceInit, Callbacks);
    }
    else {
        ((PFN_WDFDRIVERERRORREPORTAPIMISSING) WdfFunctions[WdfDriverErrorReportApiMissingTableIndex])(WdfDriverGlobals, WdfGetDriver(), NetFrameworkExtensionName, NetDeviceInitSetPowerPolicyEventCallbacksTableIndex, FALSE);
    }
#endif
}
...
#else
    if (NET_IS_FUNCTION_AVAILABLE(NetDeviceInitSetResetCapabilities)) {
        ((PFN_NETDEVICEINITSETRESETCAPABILITIES) NetFunctions[NetDeviceInitSetResetCapabilitiesTableIndex])(NetDriverGlobals, DeviceInit, ResetCapabilities);
    }
    else {
        ((PFN_WDFDRIVERERRORREPORTAPIMISSING) WdfFunctions[WdfDriverErrorReportApiMissingTableIndex])(WdfDriverGlobals, WdfGetDriver(), NetFrameworkExtensionName, NetDeviceInitSetResetCapabilitiesTableIndex, FALSE);
    }
#endif
}
...
#else
    if (NET_IS_FUNCTION_AVAILABLE(NetDeviceStoreResetDiagnostics)) {
        ((PFN_NETDEVICESTORERESETDIAGNOSTICS) NetFunctions[NetDeviceStoreResetDiagnosticsTableIndex])(NetDriverGlobals, Device, ResetDiagnosticsSize, ResetDiagnosticsBuffer);
    }
    else {
        ((PFN_WDFDRIVERERRORREPORTAPIMISSING) WdfFunctions[WdfDriverErrorReportApiMissingTableIndex])(WdfDriverGlobals, WdfGetDriver(), NetFrameworkExtensionName, NetDeviceStoreResetDiagnosticsTableIndex, FALSE);
    }
#endif
}
...
#else
    if (NET_IS_FUNCTION_AVAILABLE(NetDeviceRequestReset)) {
        ((PFN_NETDEVICEREQUESTRESET) NetFunctions[NetDeviceRequestResetTableIndex])(NetDriverGlobals, Device);
    }
    else {
        ((PFN_WDFDRIVERERRORREPORTAPIMISSING) WdfFunctions[WdfDriverErrorReportApiMissingTableIndex])(WdfDriverGlobals, WdfGetDriver(), NetFrameworkExtensionName, NetDeviceRequestResetTableIndex, FALSE);
    }
#endif
}
