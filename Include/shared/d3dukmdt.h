...
#define DXGKDDI_INTERFACE_VERSION_WDDM3_2   0x11008
...
#define D3D_UMD_INTERFACE_VERSION_WDDM3_2_2     0x11001
#define D3D_UMD_INTERFACE_VERSION_WDDM3_2       D3D_UMD_INTERFACE_VERSION_WDDM3_2_2
...
typedef enum _DXGK_PTE_BLOCK_ACCESS_MODE
{
    DXGK_PTE_BLOCK_ACCESS_NONE      = 0,
    DXGK_PTE_BLOCK_ACCESS_WRITE     = 1,
    DXGK_PTE_BLOCK_ACCESS_READWRITE = 2,
} DXGK_PTE_BLOCK_ACCESS_MODE;
...
typedef struct _DXGK_PTE
{
    union
    {
        struct
        {
            ...
            ULONGLONG BlockAccessMode       :  2;       // DXGK_PTE_BLOCK_ACCESS_MODE
...
static_assert(sizeof(DXGK_PTE) == 16, "DXGK_PTE must be 16 bytes");
#endif
...
typedef enum D3DDDI_COLOR_SPACE_TYPE
{
    ...
    D3DDDI_COLOR_SPACE_RGB_FULL_G10_NONE_P2020            = 25,
...
typedef struct D3DDDI_RESERVEGPUVIRTUALADDRESS
{
    union
    {
        D3DKMT_HANDLE               hPagingQueue;                   // Should not be used
        D3DKMT_HANDLE               hAdapter;                       // in: DXG adapter handle.
    };
    D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS          BaseAddress;     // in_opt: Base virtual address to map
    D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS          MinimumAddress;  // in_opt: Minimum virtual address
    D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS          MaximumAddress;  // in_opt: Maximum virtual address
    ...
    union
    {
        D3DDDIGPUVIRTUALADDRESS_RESERVATION_TYPE   ReservationType; // Should not be used
        UINT                                       Reserved0;
    };
    union
    {
        D3DKMT_ALIGN64 UINT64                      DriverProtection;// Should not be used
        D3DKMT_ALIGN64 UINT64                      Reserved1;
    };
    D3DKMT_ALIGN64 D3DGPU_VIRTUAL_ADDRESS          VirtualAddress;  // out: Virtual address
    union
    {
        D3DKMT_ALIGN64 UINT64                      PagingFenceValue;// Should not be used
...
typedef struct _D3DDDI_NATIVEFENCEINFO
{
    ...
    UINT                                    PhysicalAdapterIndex; // in: Index of a physical adapter (LDA) where the native fence will be resident.
    BYTE                                    Reserved[24];
} D3DDDI_NATIVEFENCEINFO;

typedef struct _D3DDDI_NATIVEFENCELOGDETAIL
{
...
typedef struct _D3DDDI_DOORBELLMAPPING
{
    D3DKMT_PTR(VOID*, CpuVa);          // out:     CPU VA of the created doorbell
    D3DKMT_PTR(VOID*, SecondaryCpuVa); // out opt: Secondary CPU VA of the created doorbell    
}D3DDDI_DOORBELLMAPPING;
...
typedef enum _DXGK_OS_FEATURE
{
    ...
    DXGK_OS_FEATURE_OPPORTUNISTIC_64KB_PAGES                = 7,
...
typedef enum _DXGK_FEATURE_ID
{
    ...
    DXGK_FEATURE_EXTENDED_SEGMENT_FLAGS             = DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_EXTENDED_SEGMENT_FLAGS),
    DXGK_FEATURE_FAULT_AND_STALL                    = DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_FAULT_AND_STALL),
    DXGK_FEATURE_SINGLE_ADAPTER_HYBRID_MODE         = DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_SINGLE_ADAPTER_HYBRID_MODE),
    ...
    DXGK_FEATURE_UNIFIED_SCHEDULING_MODEL           = DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_UNIFIED_SCHEDULING_MODEL),
    DXGK_FEATURE_NOTIFY_RESIDENCY2                  = DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_NOTIFY_RESIDENCY2),
    //DXGK_FEATURE_P2P_FENCE_SIGNAL                   = DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_P2P_FENCE_SIGNAL),
    //DXGK_FEATURE_PROCESS_DEBUG_BLOB_COLLECTION      = DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_PROCESS_DEBUG_BLOB_COLLECTION),
    DXGK_FEATURE_PANEL_BUFFER_CONTROL               = DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_DRIVER, DXGK_DRIVER_FEATURE_PANEL_BUFFER_CONTROL),
    ...
    DXGK_FEATURE_OPPORTUNISTIC_64KB_PAGES           = DXGK_DEFINE_FEATURE_ID(DXGK_FEATURE_CATEGORY_OS, DXGK_OS_FEATURE_OPPORTUNISTIC_64KB_PAGES),
...
#ifndef DXGK_FEATURE_EXTENDED_SEGMENT_FLAGS_VERSION
#define DXGK_FEATURE_EXTENDED_SEGMENT_FLAGS_VERSION DXGK_FEATURE_EXTENDED_SEGMENT_FLAGS_VERSION_APERTUREPRESERVEDDURINGSTANDBY
#endif
...
typedef struct _D3DDDI_CREATEHWQUEUEFORUSERMODESUBMISSION_FLAGS
{
    union
    {
        struct
        {
            ...
            UINT    NoBroadcastWait                : 1; // 0x00000004
            UINT    NoKmdAccess                    : 1; // 0x00000008
            UINT    TestQueue                      : 1; // 0x00000010
            UINT    DoorbellRequiresSecondaryCpuVA : 1; // 0x00000020 
...
#if defined(__cplusplus) && !defined(SORTPP_PASS)
static_assert(sizeof(D3DDDI_CREATEHWQUEUEFORUSERMODESUBMISSION_FLAGS) == sizeof(UINT), "D3DDDI_CREATEHWQUEUEFORUSERMODESUBMISSION_FLAGS must be 32 bytes");
#endif
...
typedef enum _D3DDDI_TESTCOMMANDBUFFEROP
{
    ...
    D3DDDI_TESTCOMMANDBUFFEROP_FAULT_AND_STALL              = 5,
